...
const ChatInterface: React.FC = () => {
  ...
  const initializeSession = async () => {
    try {
      const response = await axios.post('/api/v1/horoscope/chat/session');
  ...
  const sendMessage = async (message: string) => {
    ...
    try {
      const response = await axios.post('/api/v1/horoscope/chat/analyze', {
        sessionId,
        question: message,
  ...
  const handleZodiacSelect = async (zodiacInfo: string, sessionId?: string) => {
    ...
    try {
      // 首先保存星座信息到会话
      if (sessionId) {
        await axios.post('/api/v1/horoscope/chat/set-zodiac', {
          sessionId: sessionId,
          zodiac: zodiac
        });
